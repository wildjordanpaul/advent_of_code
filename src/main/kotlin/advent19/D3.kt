...
class D3 : AdventSolution(
    ...
    private fun Map<Point, Polarity>.intersections() = entries.
        filter { it.value.polarFlag == null && it.key != Point(0,0) }

    private fun String.toGrid(): MutableMap<Point, Polarity> {
        val grid = mutableMapOf<Point, Polarity>()
        split('\n').forEachIndexed { i, wire ->
            val polarity = i == 0
            var p = Point(0,0)
            var step = 0
            wire.split(',').forEach { path ->
    ...
    private fun MutableMap<Point, Polarity>.setPolarity(p: Point, steps: Int, polarFlag: Boolean) {
        val currentPolarity = this[p]
        when {
            currentPolarity == null -> this[p] = Polarity(polarFlag, steps)
            currentPolarity.polarFlag == null -> return
            currentPolarity.polarFlag != polarFlag -> this[p] = Polarity(null, currentPolarity.totalSteps + steps)
        }
    }

    private fun MutableMap<Point, Boolean?>.print() {
        keys.rangeY().forEach { y ->
            var row = ""
            keys.rangeX().forEach { x ->
                val p = Point(x, y)
                if (!containsKey(p))
                    row += "."
                else when (this[p]) {
