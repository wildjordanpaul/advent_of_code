...
import shared.splitLongs

class D9 : AdventSolution(
    mapOf(
        "104,1125899906842624,99" to  "1125899906842624",
        "1102,34915192,34915192,7,4,7,99,0" to "1219070632396864",
        "109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99" to "109"
    ...
    override fun solveProblem1(input: String): Any? {
        val amp = Amplifier(input)
        return amp.execute(1)
    }
    ...
    private class Amplifier(
        ...
        lateinit var program: MutableMap<Long, Long>
        lateinit var output: MutableList<Long>
        ...
        fun reset() {
            program = rawProgram.splitLongs().
                mapIndexed { index, l -> index.toLong() to l }.
        ...
        fun execute(programInput: Long): Long {
            while(true) {
                ...
                when(val opcode = instruction.takeLast(2).toInt()) {
                    1 -> {
                        program[program[pointer+3] ?: 0] = val1 + val2
                        pointer += 4
                    }
                    2 -> {
                        program[program[pointer+3] ?: 0] = val1 * val2
                    ...
                    3 -> {
                        program[program[pointer+1] ?: 0] = programInput
                        pointer += 2
                    }
                    4 -> {
                        output.add(val1)
                    ...
                    7 -> {
                        program[program[pointer+3] ?: 0] = if(val1 < val2) 1L else 0L
                        pointer += 4
                    }
                    8 -> {
                        program[program[pointer+3] ?: 0] = if(val1 == val2) 1L else 0L
        ...
        private fun valueAt(instruction: String, pointer: Long, offset: Int): Long {
            val position = instruction.length-2-offset
            return when(if(position < 0) '0' else instruction[position]) {
                '0' -> program[program[pointer+offset] ?: 0] ?: 0
                '2' -> program[program[pointer+offset+relativeBase] ?: 0] ?: 0
                else -> program[pointer+offset] ?: 0
            }
        }
    }
